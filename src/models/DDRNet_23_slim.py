...
class DualResNet(nn.Module):
    ...
    def forward(self, x):
        ...
        if self.augment: 
            ...
            out_x = F.interpolate(x_, size=[height_input, width_input], mode='bilinear', align_corners=True)    
            return out_x  , torch.max(out_x, 1)[1]    

        #out_val = torch.max(out, 1)[1]    

def DualResNet_imagenet(pretrained=False):
    model = DualResNet(BasicBlock, [2, 2, 2, 2], num_classes=3, planes=32, spp_planes=128, head_planes=64, augment=False)
    if pretrained:
        checkpoint = torch.load('./checkpoints/' + "DDRNet23s_imagenet.pth", map_location='cpu') 
        '''      
